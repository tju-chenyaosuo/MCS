...
class CsmithConfVecGenerator:
    ...
    @staticmethod
    def gen_single_rand_vec():
        return [random.randint(0, 100) for _ in range(single_tune_num)]
        pass

    @staticmethod
    def gen_group_rand_vec():
        group_vec = []
        for _ in tune_group_nums:
            gv = CsmithConfVecGenerator.simulate_dice(box_cnt=_, box_sizes=[100 for __ in range(_)],
                                                               game_time=100, default_cnt=0)
            group_vec.extend(CsmithConfVecGenerator.sum_up_vec(gv))
        return group_vec[:]
    ...
    @staticmethod
    def gen_rand_vec():
        single_vec = CsmithConfVecGenerator.gen_single_rand_vec()
        group_vec = CsmithConfVecGenerator.gen_group_rand_vec()
        all_vec = single_vec + group_vec
        assert len(all_vec) == 71 - len(CsmithConfigurationConf.untune_options)
        assert len(all_vec) == len(EnvironmentConf.action_mapping)
        vec_71 = [0 for _ in range(71)]
        for _ in range(len(EnvironmentConf.action_mapping)):
            vec_71[EnvironmentConf.action_mapping[_]] = all_vec[_]
        print(vec_71)
        return vec_71

    @staticmethod
    def test_():
        print(CsmithConfVecGenerator.gen_rand_vec())
...
class MlEnvironment:
    ...
    def gen_conf(self, model):
        ...
        if model is None:
            ...
            conf_vec = CsmithConfVecGenerator.gen_rand_vec()
            self.base_itm_single, self.base_itm_group = CsmithConfiguration.get_items(conf_vec)
        ...
        # a non-random step
            ...
            for _ in range(main_configure_ml.candidate_configuration_num):
                ...
                conf_vec = CsmithConfVecGenerator.gen_rand_vec()
                configuration_features.append(conf_vec)
            ...
            # predict
            conf_vec, predict_prob = self.recommend_conf(model, np.array(configuration_features), configuration_features)
            ...
            assert not os.path.exists(target_conf)
            self.base_itm_single, self.base_itm_group = CsmithConfiguration.get_items(conf_vec)
            CsmithConfiguration.write_conf(target_conf, self.base_itm_single, self.base_itm_group)
