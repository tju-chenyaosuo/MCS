    ...
    def do_dol(self):
        ...
        for o in self.opts:
            ...
            if res != LLVMTestState.success:
                ...
                ub_res = lubc.has_ub()
                # We should not assume the result of ub_checker always success
                # assert ub_res in [LLVMUBCheckerState.ub, LLVMUBCheckerState.normal]
...
class LLVMInfo:
    def __init__(self, llvm_bin_path):
        assert isinstance(llvm_bin_path, str), 'llvm_bin_path should be a str'
        llvm_bin_path = common_base_funs.add_sep(llvm_bin_path)
        self.llvm_opt = llvm_bin_path + 'opt'
        self.llvm_clang = llvm_bin_path + 'clang'
        self.llvm_llvm_as = llvm_bin_path + 'llvm-as'
    ...
    def get_ox_opts(self, level):
        cmd = ' '.join([self.llvm_llvm_as, '<', '/dev/null', '|',
                        self.llvm_opt, level, '-disable-output', '-debug-pass=Arguments', '2>&1'])
        opt_list = common_base_funs.execmd(cmd).split('\n')[:2]
...
def multi_process_test_via_conf(conf_id):
    ...
    while not all_exe:
        ...
        all_exe = program_info_queue.qsize() == EnvironmentConf.program_cnt
    # assert todo_queue.empty()    this is for multi-thread program, not multi-processing!
    # result
