...
if __name__ == '__main__':
    ...
    while tm_spt <= MainProcessConf.run_time_limit:
        ...
        if update_model:
            ...
            clu.update(buffer_a, buffer_s, buffer_r, s, reset)    # TODO be careful! param reset
            buffer_a = []
            buffer_r = []
            buffer_s = []
            iteration_cnt = 0
        ...
        if reset:
            ...
            if EnvironmentConf.global_reset_type == EnvironmentConf.global_reset_types[0]:  # default-ase-sample
                s_ = env.reset(EnvironmentConf.reset_type[5])  # default-ase-sample
            if EnvironmentConf.global_reset_type == EnvironmentConf.global_reset_types[1]:  # default-ase-ga
                s_ = env.reset(EnvironmentConf.reset_type[2])  # ga
            end_time = time.time()
